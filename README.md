# Привет! Меня зовут FanFaiks, я Технический Администратор/Разработчик и 3D моделлер в сфере Minecraft серверов

Я начал заниматься этим в 2022 году и с тех пор многому научился. Работал как над своими проектами, так и помогал другим командам. Стараюсь писать хороший, понятный код и делать качественные модели.

## 🔗Вот что я умею:
*   Писать и настраивать плагины для Minecraft (Spigot, Paper, Velocity).
*   Настраивать и администрировать серверы (VDS под Linux), управлять сетями серверов через Velocity.
*   Настраивать автоматическое резервное копирование и защиту от сбоев.
*   Создавать и анимировать 3D-модели в Blockbench.
*   Полностью собирать и настраивать игровые серверы.

## 💻Примеры моих плагинов:

1. ⏱ [EasyCooldown](https://github.com/FanFaiks/EasyCD)
Это система для установки кул даунов на предметы. Она поддерживает HEX-цвета, разные способы отображения и легко настраивается через конфиг.

2. ⚔️ [StrengthSMP](https://github.com/FanFaiks/StrengthSMP)
Это русская версия механик популярного западного проекта StrengthSMP (сезон 4). Я воссоздал все уникальные способности, 3D-элементы и ограничения оригинала.
###### Сейчас я также работаю над режимом быстрых королевских битв, который выложу сюда чуть позже.

## Примеры 3D моделей

<img width="250" height="250" alt="Потка" src="https://github.com/user-attachments/assets/f719e5c7-320f-4c44-9d73-5e7f6934d58c" />

<img width="250" height="250" alt="Световой меч" src="https://github.com/user-attachments/assets/9c224a34-ade2-4fef-b168-6f1daa383a53" />

<img width="250" height="250" alt="Меч Ареса" src="https://github.com/user-attachments/assets/9cafab53-25f1-4f7f-af03-3520e7336928" />

<img width="250" height="250" alt="Клешня" src="https://github.com/user-attachments/assets/ee860807-9dd0-461b-ada3-d87564964673" />

<img width="250" height="250" alt="ДронДоставки" src="https://github.com/user-attachments/assets/af6de210-221b-4d11-ad03-7f69fc664742" />

<img width="250" height="250" alt="Босс вибрации" src="https://github.com/user-attachments/assets/14de2c02-d0df-4cc3-b9c4-25c383bc29b8" />

## ⭐Отзывы заказчиков:
1.
<img width="486" height="74" alt="Отзыв12" src="https://github.com/user-attachments/assets/5defccc0-1490-417e-ac43-0d0681cfc1c9" />

2.
<img width="1120" height="360" alt="отзыв2" src="https://github.com/user-attachments/assets/614bdc69-15a4-49cd-acde-9d3001615904" />


## 📘Контактная информация:
*   Discord: fanfaiks
*   Telegram: @Frostele
